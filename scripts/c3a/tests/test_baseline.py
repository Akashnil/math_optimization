...
COST: count_diffset at (d=130, T=247) takes approximately 85-150 minutes.
...
the G2026g record: theta_lo >= 1.1768118.
"""
...
class TestBaselineReproduction:
    ...
    EXPECTED RUNTIME: ~85-150 minutes (count_diffset dominates).
    ...
    def test_baseline_diffset(self):
        ...
        WARNING: This takes approximately ~85-150 minutes.
        """
    ...
    def test_baseline_theta_lo(self):
        """Full build_certificate for baseline gives theta_lo >= 1.1768118.

        WARNING: This takes approximately ~85-150 minutes (diffset is the bottleneck).
        """
        mU = max_U(BASELINE_B, BASELINE_A, BASELINE_D, BASELINE_T)
        q = 2 * mU + 1
        theta_lo, theta_hi = certified_bound(BASELINE_S, BASELINE_DD, q)
        assert theta_lo >= Decimal("1.1768118"), (
            f"theta_lo={theta_lo} < 1.1768118"
        )
